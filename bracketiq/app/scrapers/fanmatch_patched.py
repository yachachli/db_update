"""
Vendored FanMatch from kenpompy with defensive fix for PredictedMOV.
...
def _safe_pred_mov(parts) -> float:
    """Compute predicted MOV from split score; return nan if not a length-2 sequence."""
...
class FanMatch:
    ...
    def __init__(self, browser: CloudScraper, date: Optional[str] = None):
        ...
        # Patched: avoid TypeError when x is float (e.g. NaN from extract) in some pandas/table cases
        fm_df["PredictedMOV"] = [
            _safe_pred_mov(x) for x in fm_df.PredictedScore.astype("str").str.split("-")
        ]
