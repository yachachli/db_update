...
def update_player_game_stats(stats_dict, player_id, cur):
    if not stats_dict:
        logging.warning(
    ...
    for game_id, stats in stats_dict.items():
        if not isinstance(stats, dict):
            logging.info(f"Stats for game {game_id} is not a dictionary. Skipping.")
            continue
...
def update_player_injuries(injury_list, cur):
    if not injury_list:
        logging.warning(f"no injury data available for {injury_list}")
        return
    player_injuries = {}
...
def update_player_info(player_data, cur):
    if not player_data.get("nbaComHeadshot", ""):
        logging.warning(f"No player info for {player_data}")
        return
    cur.execute(
        """
        UPDATE nba_players
...
def update_player_season_stats(player_data, cur):
...
def update_team_stats(team_name, team_data, cur):
...
def main():
    ...
    try:
        ...
        if injury_list:
            ...
            with get_db_connection() as conn, conn.cursor() as cur:
                update_player_injuries(injury_list, cur)
            conn.commit()
        else:
